...
α(o) = x                                                               & model formation function                                                           & Funktion zur Bestimmung der Pilz-Eigenschaften                                                                       &  &  \\
y(x) (Ziel y(x) = c(x))                                                & model function/ classifier                                                         & Bestimmung ob essbar anhand der Eigenschaften                                                                        &  &  \\
\textit{γ(o) mit O → C}                                                & ideal target function / ideal classifier für O                                     & Pilzexperte der Pilze bestimmt                                                                                       &  &  \\
c(x) mit c: X → C                                                      & ideal target function/ ideal classifier for X ; indirekt gegeben durch D           & Theoretische Ideale Bestimmung aller Pilze                                                                           &  &  \\
h(x), h: X → \{0,1\}                                                   & hypothesis/ model function  to approximate c(x)                                    & wenn es sonnig ist mache ich Sport (Regeln/ Hypothesen  zur classification)                                          &  &  \\
yi                                                                     & ground truth for xi ∈ X                                                            & der Mietpreis für eine Wohnung                                                                                       &  &  \\
\textit{H}                                                             & hypothesis space                                                                   & Menge alle möglichen Hypothesen/Regeln                                                                               &  &  \\
\textit{VH,D = \{h(x) | h(x) ∈ H ∧ ( ∀(x, c(x)) ∈ D : h(x) = c(x)) \}} & version space : Menge aller möglichen Regeln (h(x)) die sich aus D ableiten lassen & Wenn es sonnig ist gehe ich raus, Wenn es warm ist gehe ich raus, wenn es windig ist und scheint gehe ich raus, etc. &  &  \\
\textit{\#}                                                            & true error                                                                         & die summe aller falsch klassifizieren x durch Gesamtzahl aller x                                                     &  &  \\
\textit{X ∈ X / C ∈ C}                                                 & random variables                                                                   & ein random Pilz bzw. eine random Klasse                                                                              &  &  \\
p(x, c) = P(X=x, C=c)                                                  & the probability of the joint event that x ∈ X \& x belongs to class c ∈ C          & die Wahrscheinlichkeit das ein Pilz zu einer bestimmten Klasse gehört                                                &  &  \\
η                                                                      & {\ul learning rate}                                                                & a small positiv constant                                                                                             &  &  \\
                                                                       & p-dimensional direction vector w\textasciicircum →  = (w1, . . . , wp) T           &                                                                                                                      &  &  \\
